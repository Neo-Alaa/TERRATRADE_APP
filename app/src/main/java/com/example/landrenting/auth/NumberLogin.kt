...
class NumberLogin : AppCompatActivity() {
    ...
    private fun checkPhoneNumberValidity(
        ...
        val isStart = when (countryCode) {
            "+212" -> phoneEditText.text.startsWith('6') || phoneEditText.text.startsWith('7')
        ...
        if (isStart && phoneEditText.text.length == 9) {
            ...
            btnB.background = ContextCompat.getDrawable(
                this,
                R.drawable.b
            ) // Set button B background color to red
        } else {
            btnB.isClickable = false
            btnB.setBackgroundResource(R.drawable.a_layout)
